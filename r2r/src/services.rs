...
unsafe impl Send for UntypedServiceRequest {}

impl UntypedServiceRequest
{
    /// Complete the service request, consuming the request in the process.
    pub fn respond(self, msg: WrappedNativeMsgUntyped) -> Result<()> {
        let service = self
            .service
            .upgrade()
            .ok_or(Error::RCL_RET_ACTION_SERVER_INVALID)?;
        let mut service = service.lock().unwrap();
        let native_msg = WrappedNativeMsgUntyped::from(msg);
        service.send_response(self.request_id, Box::new(native_msg))
    }
}
...
impl Service_ for UnTypedService
